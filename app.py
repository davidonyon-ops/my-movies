...
# Initialize memory before widgets are created
...
if page == "Movie List":
    ...
    # REMOVED 'value=' from widgets that use keys to prevent conflict
    ...
    with st.sidebar.popover("📂 Filter by CSV Name", use_container_width=True):
        temp_selected = []
        for l in lists:
            # We use a unique key for each checkbox but reference the list memory
    ...
    if st.sidebar.button("Search & Add"):
        if add_search_query:
            ...
            if res.get("Response") == "True":
                ...
                requests.post(FORM_URL, data={ENTRY_ID_TITLE: res.get("Title"), ENTRY_ID_SOURCE: smart_source, ENTRY_ID_CONST: "MANUAL"})
